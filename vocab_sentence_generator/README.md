...
第一优先级:
  - 添加为每次生成的配置文件单独建立个新的文件夹的功能(命名按照时间排序)
  - 添加一次生成多少天的功能(用户可以选择一口气生成多少天的markdown，也可以一次只生成一天)
  - 添加可拆分单词本的功能(解决单词量过大生成时间过长)
  - 增加生成结果预览与单组重试

第二优先级:
  - 用词义相似度、主题聚类或语法模板改进分组算法
  - 增加 Prompt 模板编辑器(微调语境，方便用户选择使用场景)
  - 增加后台线程和进度条，避免长时间 AI 调用阻塞 UI (基于python可能不好实现)
  - 添加词汇本功能(复习已经学过的单词)
  - 增加本地模型或其他 AI Provider 适配器

第三优先级:
  - 增加 Anki CSV / TSV 直接导出
  - 添加在可交互界面输入API和模型的功能(最好是直接的选单-方便用户快速切换模型)
  - 添加可选的AI生图功能(需要提示可能会时间很长，以及花费时间更久)
